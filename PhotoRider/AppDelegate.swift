...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    fileprivate let locationSaver = LocationSaver()
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        // Create a context in order to write in it.
        // Do not write directy to the viewContext, it is reserved to read only.
        let context = NSManagedObjectContext(concurrencyType: .privateQueueConcurrencyType)
        context.parent = self.persistentContainer.viewContext
        
        // Get the publisher for all the new locations.
        let locationPublisher = self.locationTracker.location.eraseToAnyPublisher()
        ...
        AppDelegate.state.photoDownloader = self.photoDownloader
        AppDelegate.state.locationSaver = self.locationSaver
        
        do {
            try AppDelegate.state.restart()
        } catch {
            debugPrint("AppDelegate: can not start the ApplicationState services.")
            return false
        }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "photorider")
